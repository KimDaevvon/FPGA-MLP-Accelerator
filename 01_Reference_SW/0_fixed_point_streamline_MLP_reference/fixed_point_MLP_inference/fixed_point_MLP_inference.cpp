...
int main() {
    ...
    */
    ...
    for (int num = 1; num < num_files + 1; num++) {
        ...
        {
            snprintf(filename, sizeof(filename), "./166_label_0.txt");
    ...
    auto duration = std::chrono::duration_cast<std::chrono::microseconds>((stop - start) / num_files);
    std::cout << "Inference Time per 1 image (μs): " << duration.count() << std::endl;
